...
0. [Testing Prerequisites](#0-testing-prerequisites)
...
10. [Refactoring Changelog](#10-refactoring-changelog)
11. [Overall Feedback](#11-overall-feedback)

---

## 0. Testing Prerequisites

### Required Software
- **Python 3.12+** - For running unit tests and adapter development
- **Docker Desktop 4.x** - For running containerized services
- **Node.js 20+** - For UI service development (optional for testing)
- **pytest 8.x** - For running test suite
  ```bash
...
# Optional: Install test-specific dependencies
...
```bash
cp .env.example .env
# Required for module system:
export MODULE_ENCRYPTION_KEY=$(python -c "from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())")
# Optional API keys:
export OPENWEATHER_API_KEY=your_key_here
export CLASH_ROYALE_API_KEY=your_key_here
export CLASH_ROYALE_PLAYER_TAG=%23YOUR_TAG
```
...
# Expected output: All services return HTTP 200 or healthy status
```

### Test Data

Some tests require sample data:
- Finance tests: CSV files in `dashboard_service/Bank/`
- ChromaDB tests: No pre-populated data (created during tests)
- Module tests: Synthetic data generated by test adapters
...
```
tests/
├── unit/              # Isolated component tests (no Docker required)
├── integration/       # Multi-service tests (requires Docker)
├── functional/        # User workflow tests
└── service-isolation/ # Service startup/health tests
```
...
These tests cover:
- Delegation manager logic
- LLM client pool routing
- Model registry lookups
- Context compaction algorithms
- Finance query parsing
- Recovery mechanisms
...
# Or directly: pytest tests/integration/ -v
```

These tests cover:
- Orchestrator + LLM service interaction
- Tool calling workflows
- Sandbox code execution
- Self-consistency verification
- Module loader + registry persistence
...
```bash
make test-e2e
# Runs orchestrator E2E, tool calling, sandbox tests
```

These tests cover:
- Complete user workflows (query → tools → response)
- Module discovery and enablement
- Credential hot-reload workflow
...
```bash
make showroom
# 15+ endpoint checks with pass/fail reporting
```

The showroom validates:
- ✓ Admin API module CRUD
...
[PASS] Orchestrator admin API
[PASS] Context aggregation
[PASS] Pipeline SSE stream
[PASS] Module listing
...
...
**Example**:
```python
def test_delegation_manager_classification():
    manager = DelegationManager(threshold=0.7)
    tier = manager.classify_complexity("What's 2+2?")
...
def test_admin_api_module_enable(admin_client):
    response = admin_client.post("/admin/modules/weather/openweather/enable")
...
**Example**:
```python
def test_chat_flow_with_tools(orchestrator_client):
    # User sends query
    response = orchestrator_client.execute_task("What's the weather?")
    # Orchestrator calls weather tool
    assert "tool_calls" in response
    # Returns formatted answer
    assert "temperature" in response.result.lower()
```
...
def test_my_feature_success():
    ...
    result = my_feature.process({"input": "test"})
...
```bash
pytest --cov=shared --cov=orchestrator --cov=dashboard_service --cov-report=html
...
**Current Status**: Tests run manually before commits.

**Planned** (Q2 2026): GitHub Actions workflow for:
- Automatic test runs on PR
- Coverage reporting
- Failed test notifications
- Integration test matrix (Python 3.11, 3.12, 3.13)
...
2. **Mock External APIs**: Never call real APIs in tests
   ...
   def mock_weather_api(monkeypatch):
       ...
       monkeypatch.setattr(httpx.AsyncClient, "get", mock_fetch)
   ```

3. **Test Edge Cases**: Don't just test the happy path
   ```python
   def test_empty_input():
       assert my_func("") == default_value
...
4. **Use Descriptive Names**: Test names should explain what they test
   ```python
   # Good
   def test_adapter_handles_api_timeout():
       ...
...
### 7.8 Troubleshooting Test Failures
...
**Error**: `AssertionError: Expected 0 transactions, got 5`

**Solution**: Clear test databases before running:
```bash
...
### 11.1 Feature Priority Assessment
...
### 11.6 Suggestions for Improvement
