...
class RouterConfig:
    ...
    max_tokens: int = 256  # Reduced from 512 for faster routing
    temperature: float = 0.1  # Low temperature for deterministic routing
    n_ctx: int = 4096  # Increased from 2048 to match main model
    n_threads: int = 4
...
class Router:
    ...
    def _get_json_grammar(self) -> str:
        """
        Get JSON grammar for llama-cli to constrain output.
        
        Returns:
            str: GBNF grammar for valid JSON
        """
        return '''
root ::= object
...
'''
    ...
    def route(self, query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Build llama-cli command with JSON grammar constraint
            cmd = [
                ...
                "-ngl", "0",  # Disable GPU layers for consistency
                "--grammar", self._get_json_grammar(),  # Force valid JSON
            ]
    ...
    def _parse_router_output(self, output: str) -> Dict[str, Any]:
        ...
        """
        # Find first complete JSON object in output
        ...
        # Parse JSON character by character to find complete object
        brace_count = 0
        in_string = False
        escape_next = False
        json_end = json_start
        ...
        if brace_count != 0:
            logger.warning("Incomplete JSON object in router output")
